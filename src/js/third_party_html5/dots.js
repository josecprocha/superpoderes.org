...
const Dot = Vue.component('dot', {
  ...
  data() {
    const startAnimationDelay = 0.25 + (0.25 * (this.row * this.col)) / this.$store.state.maxScore;
    return {
      style: {
    ...
    gridStyle() {
      ...
      return {
        ...
        gridTemplateColumns: `repeat(${this.cols -
          1}, ${nodeWeight} ${spacingWeight}) ${nodeWeight}`,
...
const NewGame = Vue.component('new-game', {
  ...
  computed: {
    player1: function() {
      return this.$store.getters.playerName(1);
    },
    player2: function() {
      return this.$store.getters.playerName(2);
    }
  },
...
function isAdjacent(first, second) {
  return (
    (first.row === second.row && (first.col === second.col - 1 || first.col === second.col + 1)) ||
...
const store = new Vuex.Store({
  ...
  getters: {
    isSelectedDot(state) {
      return ({ row, col }) =>
        state.firstDot && state.firstDot.row === row && state.firstDot.col === col;
    },
    isDrawnEdge(state) {
...
const routes = [{ path: '/', component: NewGame }, { path: '/game', component: DotGame }];
...
class Sound {
  ...
  onLoad(request) {
    this.audioContext.decodeAudioData(request.response, buffer => (this.audioBuffer = buffer));
  }
...
const collect = new Sound(AUDIO_CONTEXT, isMuted).load(
  'https://s3-us-west-2.amazonaws.com/s.cdpn.io/928004/collect.mp3'
);
const tap = new Sound(AUDIO_CONTEXT, isMuted).load(
  'https://s3-us-west-2.amazonaws.com/s.cdpn.io/928004/tap-1.mp3'
);
