...
sounds.fail = new Howl({
  src: ['https://cdn.rawgit.com/cheapjoe/filehost/f77a5925/mp3/fail-buzzer-02.mp3'],
  volume: 0.25
});
sounds.fail.rate(4);
...
var _snake = function(snakeLength) {
  ...
  this.draw = function() {
    ...
    for (var i = 0; i < this.elements.length - 1; i++) {
      ...
      ctx.rect(
        this.elements[i].x - 0.5 * config.bodySize,
        this.elements[i].y - 0.5 * config.bodySize,
        config.bodySize,
        config.bodySize
      );
      ctx.fill();
    ...
    ctx.beginPath();
    ctx.arc(
      this.elements[this.elements.length - 1].x,
      this.elements[this.elements.length - 1].y,
      config.headSize,
      0,
      2 * Math.PI
    );
...
var _game = function() {
  ...
  this.checkCollision = function() {
    if (
      this.snake.elements[this.snake.elements.length - 1].x <= 0 ||
      this.snake.elements[this.snake.elements.length - 1].x >= config.w ||
      this.snake.elements[this.snake.elements.length - 1].y <= 0 ||
      this.snake.elements[this.snake.elements.length - 1].y >= config.h
    ) {
      fail();
    }
    ...
    for (var i = 0; i < this.snake.elements.length - 1; i++) {
      if (
        this.snake.elements[this.snake.elements.length - 1].x == this.snake.elements[i].x &&
        this.snake.elements[this.snake.elements.length - 1].y == this.snake.elements[i].y
      ) {
        fail();
      }
    }
    ...
    if (
      this.snake.elements[this.snake.elements.length - 1].x == this.apple.x &&
      this.snake.elements[this.snake.elements.length - 1].y == this.apple.y
    ) {
      sounds.eat.play();
...
(function() {
  ...
  gui
    ...
    .max(30);
  gui
    .add(config, 'growAmount')
    .min(0)
    .step(1)
    .max(20);
  gui
    .add(config, 'initialLength')
    .min(2)
    .step(1)
    .max(10)
