var tictactoeApp = function() {
  ...
  function checkWin(moves) {
    ...
    if (moves.length >= 3) {
      // can't win with less than 3 squares
      ...
      if (moves.length === 5 && confirmWin < 3) {
        //Draw Game
        endGameDraw();
      }
    } //endif
  ...
  function updateAiMovesList(updateAi) {
    if (oMoves.includes(updateAi) === false && xMoves.includes(updateAi) === false) {
      // make sure square hasn't been played yet
  ...
  function adaptLoop(moves) {
    for (i = 0; i < wins.length; i++) {
      ...
      if (numWin === 2) {
        // 2 squares within a win line have been played find the missing piece for a win.
        var updateAi = wins[i];
  ...
  function gameCycle(square, letter) {
    if (numPlayers === 1 && gameOver === false) {
      // One Player Game
      if (playerLetter === 'X') {
