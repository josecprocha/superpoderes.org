...
var Ball = function() {
  (this.size = 48),
    (this.speed = 450), // original value is 600
...
var Paddle = function() {
  ...
  (this.updatePivot = function() {
    ...
    (this.render = function() {
      if (this.angle == 0) {
        // paddle doesn't need to be rotated
...
addEventListener(
  ...
  function(e) {
    ...
    if (e.keyCode == 40) {
      // P2 pressed down (key: arrow down)
      if (!keysDown[40]) {
        // execute only the first time
        p2.angle = -ROTATION_ANGLE;
        p2.rotatePoints(p2.angle);
...
addEventListener(
  ...
  function(e) {
    if (e.keyCode == 87) {
      // P1 released up (key: w)
      p1.angle = 0;
      p1.realignPoints();
    }

    if (e.keyCode == 83) {
      // P1 released down (key: s)
      p1.angle = 0;
      p1.realignPoints();
    }

    if (e.keyCode == 38) {
      // P2 released up (key: arrow up)
      p2.angle = 0;
      p2.realignPoints();
    }

    if (e.keyCode == 40) {
      // P2 released down (key: arrow down)
      p2.angle = 0;
      p2.realignPoints();
    }
