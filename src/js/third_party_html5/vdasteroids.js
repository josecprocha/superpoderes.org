...
colCheck = () => {
  ...
  $(asteroids).each(function(i) {
    ...
    var left = Math.random() * constrains;
    if (
      parseFloat(oTh.toFixed(0)) >= parseFloat(pT.toFixed(0)) &&
      parseFloat(oTh.toFixed(0)) - speeds[i] <= parseFloat(pT.toFixed(0))
    ) {
      if (
        o.offset().left < ply.offset().left.toFixed() &&
        o.offset().left + o.width() > ply.offset().left.toFixed()
      ) {
        hp(true);
      }
    }
    if (bhSpawn == true) {
      if (
        bH.offset().top < o.offset().top &&
        bH.offset().top + bH.height() > o.offset().top &&
        bH.offset()
      ) {
        if (
          o.offset().left + o.width() / 2 > bH.offset().left - o.width() / 2 &&
          o.offset().left < bH.offset().left + bH.width()
        ) {
          scores();
          resetAst(o, left);
        }
    ...
    if (
      o.offset().top < l.offset().top &&
      o.offset().top + o.height() > l.offset().top &&
      o.offset()
    ) {
      if (
        o.offset().left < l.offset().left + l.width() &&
        o.offset().left + o.width() > l.offset().left
      ) {
        if (inFire == true) {
          resetAst(o, left);
          scores();
