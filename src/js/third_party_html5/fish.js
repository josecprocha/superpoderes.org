var pFish = {
  ...
  methods : {
          	...
          	scoreAnimation : function(){
          		new TimelineLite()
                  .fromTo(this.$el.querySelector('.score-notifier'),0.8,{
                  	autoAlpha : 0,
	              	y : 0,
	              	scale : 1
                  },{
	              ...
	              .to(this.$el.querySelector('.score-notifier'),0.2,{
	              	autoAlpha : 0,
          	...
          	fishAnimation : function(){
          		new TimelineMax()
          		   .fromTo(this.$el.querySelector('.p-fish-mouth-close'),0.2,{
          		   	morphSVG : {
          	...
          	bubbles : function(){
          	 return  new TimelineLite()
          		    .staggerFromTo(this.$el.querySelectorAll(".fish>.bubbles>ellipse"),2,{
          		    	autoAlpha : 0,
          		    	x : 0,
	          			y : 0,
	          			scale : 1
	          		...
	          		.staggerTo(this.$el.querySelectorAll(".fish>.bubbles>ellipse"),0.2,{
	          			autoAlpha : 0,
	          			ease : Linear.easeNone
	          		},0.1,0.8);
          	},
          	stool : function(){
          	 return	new TimelineLite()
          		    .staggerFromTo(this.$el.querySelectorAll(".fish>.stool>path"),2,{
          		    	autoAlpha : 0,
          		    	x : 0,
	          			y : -10,
	          			scale : 2
	          		...
	          		.staggerTo(this.$el.querySelectorAll(".fish>.stool>path"),1,{
	          			cycle : {
	          				x : function(){
          	...
          	difficating : function(){
          		var el = this;
          		new TimelineMax({
          			 onComplete : function(){ el.$emit('enablemouseevent'); }
          	        })
...
var Game = new Vue({
  ...
  methods : {
            ...
            var waveLayers = this.$el.querySelectorAll('.water-layer > div');
             ...
             // 	waveLayers[i]
             // }
             TweenMax.staggerTo(waveLayers,2,{
             	cycle : {
                     y : function(i,el){
                     ...
                     var tl = new TimelineMax();
                      for (var j=0; j<bubbles;j++){
                          ...
                          TweenLite.set(bubble,{
                           ...
                           TweenMax.fromTo(bubble,Math.random()*4 + 2,
                           	{x : -20},
                           	{x : 20,repeat : -1,yoyo:true,ease : Linear.easeNone});

                           tl.to(bubble,Math.random()*3 + 2,{
			...
			coreGaming : function(){
             ...
             // Mouse events
             this.$el.getElementsByClassName('prey-fish-wrapper')[0].addEventListener('mousemove',this.mousemovemethod);
           ...
           // Testing hits
			...
			generateBubbles : function(){
          		var obj = this,
	               ...
	               tl.staggerTo(el.querySelectorAll('.prey-fish-wrapper > .fish-bubble'),2,{
	               	cycle : {
	               		x : function(){
	          					return Math.random() * 20 + 10;
	          				},
	               ...
	               .staggerTo(el.querySelectorAll('.prey-fish-wrapper > .fish-bubble'),4,{
	               	cycle : {
	          				x : function(){
	          					return Math.random() * 50 + 20;
	          				},
