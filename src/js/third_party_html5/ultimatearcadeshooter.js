//<!--
document.write('&copy;');
document.write(' TheCodersZone ');
document.write(new Date().getFullYear());
//-->
//Create canvas
//https://codepen.io/TheCodersZone/pen/aNOWrW/license
//copyright
document.body.innerHTML += '<canvas id="canvas" width="800" height="440"></canvas>';
...
function fireBullet(offset, speed) {
  ...
  bullet.position.set(
    player.position.x + player.size / 2 - bullet.size / 2 + offset.x,
...
function increaseDifficulty() {
  ...
  if (difficulty % 5 == 0 && Math.random() > 0.5) spawnPowerUp();
  clearInterval(spawnTimer);
  spawnTimer = setInterval(spawnEnemy, spawntime);
}
...
function sawtoothMotion(entity, amplitude, freq, speed) {
  ...
  entity.update = function(deltatime) {
    this.velocity.x =
      modifier * ((-2 * amplitude) / Math.PI) * Math.atan(1 / Math.tan(this.position.y / freq));
    this.velocity.y = speed;
    Entity.prototype.update.call(this, deltatime);
  };
}
...
var PowerUp = function() {
  ...
  this.size = 20;
  this.time = 0;
  this.particlesDelay = 0.25;
  var r = Math.random();
  if (r < 0.5) {
    this.effect = 'scoreBoost';
  } else if (r < 0.75) {
    this.effect = 'tripleShot';
  } else {
    this.effect = 'bomb';
  }
};
