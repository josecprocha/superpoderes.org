$(document).ready(function() {
  ...
  var MathGame = {
    assignNumber: function() {
      ...
      $toResult =
        $change === 1 || $change === 2
          ? $toFirst + $toSecond
          : Math.floor(Math.random() * 5 + ($toFirst + $toSecond + 1));
    ...
    toEnd: function() {
      ...
      $timer = 0;
      $('body')
        .removeClass('start_ON main_ON')
        .addClass('end_ON');
      $('.field.end')
        .removeClass('level1 level2 level3 level4')
  ...
  $('#withButton').on('click', function() {
    // button is active
    $('body')
      .removeClass('withButton_ON')
      .addClass('withMouse_ON');
    $buttonOrMouse = 1;
  });
  $('#withMouse').on('click', function() {
    // mouse is active
    $('body')
      .removeClass('withMouse_ON')
      .addClass('withButton_ON');
    $buttonOrMouse = 0;
  });
