...
var draughtsInitial = [
  // ' ' is an empty square, 'b' a black piece, 'w' a white piece; 'B' and 'W' are kings.
  [' ', 'b', ' ', 'b', ' ', 'b', ' ', 'b'],
  ['b', ' ', 'b', ' ', 'b', ' ', 'b', ' '],
  [' ', 'b', ' ', 'b', ' ', 'b', ' ', 'b'],
  [' ', ' ', ' ', ' ', ' ', ' ', ' ', ' '],
  ...
  draughts.forEach((row, r) => {
    ...
    row.forEach((square, c) => {
      ...
      if (legalMoves.some(m => m.r == r && m.c == c)) {
        if (humans[player]) {
          draughtsSquare.onclick = function() {
            selectPiece(r, c);
          };
      ...
      if (
        ...
        legalMoves.some(
          m =>
            m.r == selected.r &&
            m.c == selected.c &&
            m.moveSequence[0].r == r &&
            m.moveSequence[0].c == c
        )
      ) {
        if (humans[player]) {
          draughtsSquare.onclick = function() {
...
function placePiece(r, c) {
  // r = row, c = column
  var move = legalMoves.find(
    m =>
      m.r == selected.r && m.c == selected.c && m.moveSequence[0].r == r && m.moveSequence[0].c == c
  );
  ...
  if (move.moveSequence.length > 1) {
    legalMoves = legalMoves
      ...
      .map(m => ({
        r: m.moveSequence[0].r,
        c: m.moveSequence[0].c,
        moveSequence: m.moveSequence.slice(1)
      }));
    selected = { r: r, c: c };
  } else {
    selected = false;
...
function updateState(board, pieceR, pieceC, destinationR, destinationC) {
  // p = player
  ...
  if (Math.abs(destinationR - pieceR) == 2) {
...
function findLegalMoves(board, p) {
  // p = player
    ...
    row.forEach((square, c) => {
      if (square.toUpperCase() == p) {
        ...
        if (captures.length > 0 || possibleCaptures.length > 0)
          possibleCaptures = possibleCaptures.concat(
            captures.map(seq => ({ r: r, c: c, moveSequence: seq }))
          );
        else {
          var steps = findPossibleSteps(board, r, c, p);
          if (steps.length > 0)
            possibleSteps = possibleSteps.concat(
              steps.map(step => ({ r: r, c: c, moveSequence: step }))
            );
        }
      }
    });
...
function findPossibleCaptures(board, r, c, p, king) {
  ...
  if (
    c >= 2 &&
    r + 2 * dir >= 0 &&
    r + 2 * dir < 8 && // take piece on the left
    board[r + dir][c - 1] != ' ' &&
    board[r + dir][c - 1].toUpperCase() != p &&
    board[r + 2 * dir][c - 2] == ' '
  ) {
    ...
    if (furtherCaptures.length === 0) possibleCaptures.push([{ r: r + 2 * dir, c: c - 2 }]);
      ...
      possibleCaptures = possibleCaptures.concat(
        furtherCaptures.map(cap => [{ r: r + 2 * dir, c: c - 2 }].concat(cap))
      );
  }
  if (
    c <= 5 &&
    r + 2 * dir >= 0 &&
    r + 2 * dir < 8 && // take piece on the right
    board[r + dir][c + 1] != ' ' &&
    board[r + dir][c + 1].toUpperCase() != p &&
    board[r + 2 * dir][c + 2] == ' '
  ) {
...
function findPossibleSteps(board, r, c, p) {
  // r = row, c = column, p = player
  var steps = [];
  var dir = p == 'B' ? 1 : -1;
  if (c >= 1 && r + dir >= 0 && r + dir < 8 && board[r + dir][c - 1] == ' ')
    steps.push([{ r: r + dir, c: c - 1 }]);
  if (c <= 6 && r + dir >= 0 && r + dir < 8 && board[r + dir][c + 1] == ' ')
    steps.push([{ r: r + dir, c: c + 1 }]);
...
function mcMove() {
  if (mcTree.children.length === 0)
    mcTree.children = findLegalMoves(mcState, mcTree.player).map(m => ({
      r: m.r,
      c: m.c,
      moveSequence: m.moveSequence,
      player: mcTree.player == 'W' ? 'B' : 'W',
      wins: 0,
      plays: 0,
      parent: mcTree,
      children: []
  ...
  if (mcTree.player != player) {
    ...
    if (i >= lim) {
      console.log('error: mcTree not updated');
      console.log(mcTree);
      return false;
    }
  }
  var then = Date.now();
  var i = 0;
  while (Date.now() - then < 1000 && i < 1500) {
    // runs mcts for at most 1 second or 1500 iterations
    mcts();
    i++;
  }
  var score, maxI;
...
function simulateMoveSequence(board, move) {
  return move.moveSequence.reduce(
    (a, b) => ({ board: updateState(a.board, a.r, a.c, b.r, b.c), r: b.r, c: b.c }),
    { board: board.map(r => r.slice()), r: move.r, c: move.c }
  ).board;
}
...
function mcts() {
  ...
  var loser = childNode.hasOwnProperty('loser')
    ? childNode.loser
    : mcSimulation(childNode.board, childNode.node.player);
