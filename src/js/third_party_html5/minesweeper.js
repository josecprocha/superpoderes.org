...
var $game = {
  ...
  validatesInputs: function() {
    ...
    var errors = {};
    if (inputs.rows < 1 || inputs.rows > 30) {
      errors.rows = true;
    }
    if (inputs.cols < 1 || inputs.cols > 30) {
      errors.cols = true;
    }
    if (!(errors.rows || errors.cols) && (inputs.mines < 1 || inputs.mines > this.maximumMines)) {
      errors.mines = true;
    }
    return errors;
  },
...
function initControlSystem() {
  ...
  $game.startGameBtn.addEventListener('click', function(event) {
    ...
    if (nodes.rows.parentNode.hasClass('error')) {
      nodes.rows.parentNode.removeClass('error');
    }
    if (nodes.cols.parentNode.hasClass('error')) {
      nodes.cols.parentNode.removeClass('error');
    }
    if (nodes.mines.parentNode.hasClass('error')) {
      nodes.mines.parentNode.removeClass('error');
    }
