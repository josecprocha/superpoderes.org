  ...
  .controller('BoardController', [
    ...
    ($scope, Helpers) => {
      ...
      var isCoordValid = (i, j, k) => {
        return (
          i >= 0 && i < $scope.width && j >= 0 && j < $scope.height && k >= 0 && k < $scope.depth
        );
      };
      ...
      $scope.$on('clickedTile', (evt, data) => {
        ...
        if ($scope.selectedTiles.length === 2) {
          ...
          if ($scope.areTilesEqual(tile1.tile, tile2.tile)) {
            var p1 = tile1.tile.position,
              p2 = tile2.tile.position;
        ...
        for (var i = 0, len = tilesOnStage.length; i < len; i++) {
          for (var j = 0, jlen = tilesOnStage[i].length; j < jlen; j++) {
            for (var k = 0, klen = tilesOnStage[i][j].length; k < klen; k++) {
              if (tilesOnStage[i][j][k]) $scope.tiles.push(tilesOnStage[i][j][k]);
            }
          }
        }
      };
          ...
          for (i0 = -1; i0 <= 1; i0++) {
            for (j0 = -1; j0 <= 1; j0++) {
              if (isCoordValid(i + i0, j + j0, k + 1)) {
                if (tiles[i + i0][j + j0][k + 1]) return false;
              }
            }
          }
        }
      ...
      $scope.areTilesEqual = (tile1, tile2) => {
        ...
        var t2 = tile2.tile;
        // ordinary tiles
        if (t1 < 35 && t2 < 35) return t1 == t2;
        // seasons
        if (t1 >= 35 && t1 <= 38 && t2 >= 35 && t2 <= 38) return true;
        // flowers
        if (t1 >= 39 && t1 <= 42 && t2 >= 39 && t2 <= 42) return true;
