...
for (var i = 1; i <= 64; i++) block[i] = new square_p(square_class[i], i);
...
function makeMove(index) {
  var isMove = false;
  if (!selectedPiece)
    // daca jocu de abea a inceput si nu a fost selectata nicio piesa
    return false;
  ...
  if (mustAttack)
    // ca sa stiu daca sar doar un rand sau 2
    multiplier = 2;
  else multiplier = 1;
...
function executeMove(X, Y, nSquare) {
  ...
  block[the_checker[selectedPieceindex].ocupied_square + nSquare].pieceId =
    block[the_checker[selectedPieceindex].ocupied_square].pieceId;
...
function eliminateCheck(indexx) {
  if (indexx < 1 || indexx > 64) return 0;
...
function changeTurns(ckc) {
  if (ckc.color == 'white') the_checker = b_checker;
  else the_checker = w_checker;
}

function checkIfLost() {
  var i;
  for (i = 1; i <= 12; i++) if (the_checker[i].alive) return false;
  return true;
}
...
function declareWinner() {
  ...
  0;
  if (the_checker[1].color == 'white') score.innerHTML = 'Black wins';
