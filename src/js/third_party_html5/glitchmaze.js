...
function checkLevelGenFrom(head, dx, dy) {
  ...
  if (x < 0 || y < 0 || x >= temp.size || y >= temp.size) return void 0;

  var str = x + ',' + y;
  if (temp.checked.indexOf(str) === -1) return [dx, dy];
}
game.levelText = [
  'wasd hjkl ←↑↓→',
  'reach top left',
...
window.addEventListener('touchend', function() {
  ...
  for (var i = 0; i < game.controls.pressed.length; ++i) game.controls.pressed[i] = false;
});
...
game.player = {
  ...
  mx: 0.6, // to multiply speed, kinda like an opposite of friction, but easier to keep track of
  my: 0.6,
...
game.controls = {
  ...
  keyCodes: [[75, 38, 87], [72, 37, 65], [74, 40, 83], [76, 39, 68], [32, 13], [80, 77]]
};
window.addEventListener('keydown', function(e) {
  var prevent = false;
  for (var i = 0; i < game.controls.keyCodes.length; ++i) {
    if (game.controls.keyCodes[i].indexOf(e.keyCode) > -1) game.controls.pressed[i] = true;
  }
});
window.addEventListener('keyup', function(e) {
  var prevent = false;
  for (var i = 0; i < game.controls.keyCodes.length; ++i) {
    if (game.controls.keyCodes[i].indexOf(e.keyCode) > -1) game.controls.pressed[i] = false;
  }
});
function anim() {
  game.state = 'anim';
  ...
  if (game.levelNum < 4) {
    // 2^(x/2+3) = log2(512/16) when x = 4, so whole map can fit in screen as long as the level is below 4
  ...
  if (
    game.controls.pressed[4] &&
    game.player.breakpoints.length < game.player.breakpointNum &&
    game.player.ticksSinceLastBreakpoint > 30
  ) {
  ...
  if (game.player.tick < 120) {
    ...
    ctx.font =
      Math.min(
        0.3 +
          0.7 *
            ((game.player.maxSpeed - (Math.abs(game.player.vx) + Math.abs(game.player.vy))) /
              game.player.maxSpeed),
        0.75 + (game.player.tick % 60) / 240
      ) *
        16 +
      'px monospace';
  ...
  for (var x = Math.max(fx, 0); x < Math.min(fx + 32, size); ++x) {
    for (var y = Math.max(fy, 0); y < Math.min(fy + 32, size); ++y) {
      if (game.level[x][y] === WALL && Math.random() < 0.3) {
  ...
  ctx.fillText(
    ...
    (tx - 0.5 + game.player.x + game.player.ox) * 16,
    (ty - 0.5 + game.player.y + game.player.oy) * 16
  );
  ...
  for (var i = 0; i < game.player.breakpoints.length; ++i)
    ctx.fillText(
      chars[(Math.random() * chars.length) | 0],
      (tx + game.player.breakpoints[i].x) * 16,
      (ty + game.player.breakpoints[i].y) * 16
    );
...
function openMenu(addLevel) {
  ...
  for (var y = 0; y < game.glitchSchema.length; ++y) {
    for (var x = 0; x < game.glitchSchema[0].length; ++x) {
      if (game.glitchSchema[y][x] !== ' ' && Math.random() < 0.4) {
        ctx.fillStyle =
          game.glitchSchema[y][x] === 'x'
            ? '#eee'
            : 'hsl(' + ((x + y) * 4 + game.menuTick) + ',80%,50%)';
        ctx.fillText(chars[(Math.random() * chars.length) | 0], tx + x * 16, ty + y * 16);
      }
    }
  }
  ...
  if (game.ticksSinceMenuInteraction > 10) {
    ...
    if (game.controls.pressed[1] && game.levelNum > 0) {
      ...
      game.controls.pressed[0] ||
      game.controls.pressed[2] ||
      game.controls.pressed[4] ||
      game.controls.pressed[5]
    ) {
      menuDone = true;
      genLevel();
