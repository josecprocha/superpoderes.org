...
function resetGame()
{
	...
	for(var i in elementPool){ resetRow(elementPool[i]); }
	runway.innerHTML = '';
	elementsActive = [];
	worldTiles = [];
	playerScore = 0;
...
function fillWindowDown()
{
	...
	let rowID           = elementsActive.length > 0 ? parseInt( elementsActive[elementsActive.length-1].getAttribute("rowID") ) : 0 ;
	let currentBottom   = elementsActive.length > 0 ? elementsActive[elementsActive.length-1].getBoundingClientRect().bottom : 0;
	...
	for(var i = 0; i < elementsActive.length; i++)
	{
		if
			(
				...
				direction > 0 && elementsActive[i].getBoundingClientRect().bottom < -(window.innerHeight*0.25) ||
...
function getTilesInRow( row )
{
	...
	while(endIndex >= worldTiles.length){ addSection(); }
...
function revealTile(worldTile)
{
	...
	if(worldTile.mine) { gameOver(worldTile) }
	else
	{
		playerScore++;
		...
		for(var i in neighbours)  { if(neighbours[i].mine){ mineCount++; }  }
		...
		if(mineCount === 0)
		{
			...
			{
				if( !neighbours[i].revealed){ revealTile(neighbours[i]); }
			}
		}
		...
		if(worldTile.element)
		{
			...
			if(worldTile.mineCount > 0) { worldTile.element.innerHTML = '<span>'+worldTile.mineCount+'</span>' };
		}
	}
