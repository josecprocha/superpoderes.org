...
var w = 33,
  h = 33,
  maze = [],
  dirs = [{ x: -2, y: 0 }, { x: 0, y: -2 }, { x: 2, y: 0 }, { x: 0, y: 2 }],
  goal = {};
...
function genMaze() {
  ...
  do {
    var available = [
      ...
      return (
        coord.x > 0 && coord.x < w && coord.y > 0 && coord.y < h && maze[coord.x][coord.y] === 0
      );
    });
...
window.addEventListener('keydown', function(e) {
  keys.map(function(key) {
    if (key.codes.indexOf(e.keyCode) > -1) key.pressed = true;
  });
});
window.addEventListener('keyup', function(e) {
  keys.map(function(key) {
    if (key.codes.indexOf(e.keyCode) > -1) key.pressed = false;
  });
});
