...
function generate_blocks() {
  $block =
    '<div class="fg-block fg-both color" pos="P0" id="block-ID" data-x="X0" data-y="Y0"><div>text</div></div>';
  var newBlock,
    i = 0,
    color;
...
function shuffle_blocks() {
  ...
  for (var i = 0; i < shuffleMoves; i++) {
    ...
    if (holeY == numBlocksY - 1 || lastR === 'down') {
      up = false;
    }
    if (holeY == 0 || lastR == 'up') {
      down = false;
    }
    if (holeX == numBlocksX - 1 || lastR === 'right') {
      left = false;
    }
    if (holeX == 0 || lastR == 'left') {
      right = false;
    }
...
function initiate_input() {
  ...
  $('.fg-block').on('click touchstart', function() {
    ...
    if (check_win()) {
      ...
      setTimeout(function() {
        $('.message .text')
          .html('Congratulations!<br>You won in ' + moves + ' moves!')
          .show();
      }, 200);
    }
  });
...
function move_blocks(input) {
  ...
  for (var i = 1; i <= numBlocksToMove; i++) {
    if (direction == 'left') {
      newHolePos = holePosition + 1;
    }
    if (direction == 'right') {
      newHolePos = holePosition - 1;
    }
    if (direction == 'up') {
      newHolePos = holePosition + numBlocksX;
    }
    if (direction == 'down') {
      newHolePos = holePosition - numBlocksX;
    }
