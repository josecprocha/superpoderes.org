...
function Asteroid(pos, dir, level) {
  this.pos = pos.clone();
  this.rot = 0;
  this.rotSpeed = Math.random() * 0.3 - 0.15;
  this.dir = dir.clone();
  this.alive = true;
  this.onscreen = false;
  this.level = level;
...
Asteroid.prototype.hit = function(laserDir) {
  ...
  if (this.level > 0) {
    ...
    if (dir1.len() < 0.5) dir1 = dir1.mult(0.5 / dir1.len());
    ...
    if (dir2.len() < 0.5) dir2 = dir2.mult(0.5 / dir2.len());
...
Particle.prototype.update = function() {
  this.pos.add(this.dir);
  this.alive =
    --this.life > 0 && this.pos.x > 0 && this.pos.x < cw && this.pos.y > 0 && this.pos.y < ch;
};
...
var player = {
  ...
  accelerate: function() {
    this.travel.add(this.dir.mult(0.2));
  ...
  update: function() {
    ...
    this.travel = this.travel.mult(0.99);
...
function createAsteroid() {
  if (asteroids.length < 5 + player.score / 100) {
    ...
    if (dir.len() < 0.5) dir = dir.mult(0.5 / dir.len());
